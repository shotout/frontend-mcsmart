...
import { AdEventType, AppOpenAd, InterstitialAd } from "react-native-google-mobile-ads";
...
import { getUserProfile, postRegister, updateProfile } from "../../shared/request";
...
const interstialAdsLearn = InterstitialAd.createForAdRequest(getRewardedInsterstialLearnMoreID(), {
  requestNonPersonalizedAdsOnly: true,
  keywords: ["fashion", "clothing"],
});
...
function Routes({ registerData, userProfile, props }) {
  ...
  const checkMinute = (start) => {
    ...
    if (timeRemaining > 600) {
      // Jika lebih dari 600 detik (10 menit)
  ...
  useEffect(() => {
    ...
    const listenerOpenApps = appOpenAd.addAdEventListener(AdEventType.OPENED, () => {
    ...
    const listenerIAPAds = appOpenAd.addAdEventListener(AdEventType.CLOSED, () => {
    ...
    const subscription = AppState.addEventListener("change", async (nextAppState) => {
      ...
      if (appState.current.match("background") && nextAppState === "active") {
        appOpenAd.load();
        if (paywallStatus && paywallStatus.current !== "PRESENTATION_CLOSED") {
          if (Platform.OS === "ios") {
            handleLoadInAppAds();
          }
  ...
  function getInitialRoute() {
    if (
      ...
      ((registerData?.registerStep === 7 || registerData?.registerStep === 8) && checking() !== "0" && !isUserPremium())
    ) {
      return "MainPage";
    }
    if (registerData) {
    ...
    <View style={{ flex: 1, position: "relative" }}>
      <NavigationContainer ref={navigationRef} linking={navigationLinking}>
        <Stack.Navigator initialRouteName={getInitialRoute()}>
          <Stack.Screen options={navigationData.noHeader.options} name='WelcomePage' component={WelcomePage} />
          <Stack.Screen options={navigationData.noHeader.options} name='Register' component={Register} />
          <Stack.Screen options={navigationData.noHeader.options} name='MainPage' component={MainPage} />
        </Stack.Navigator>
      </NavigationContainer>
